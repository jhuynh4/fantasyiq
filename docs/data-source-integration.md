...
| Player game log (per-game stats) | `GET https://site.web.api.espn.com/apis/common/v3/sports/football/nfl/athletes/{athlete_id}/gamelog` |
...
- **ESPN's injury data is genuinely thin.** Budget time in Phase 2 to test how much real signal you get from roster/news-derived status before assuming it's as good as a paid source's practice-participation detail — this is the one place your free-tier choice has a real, known downside (flagged back in the source-selection decision).
