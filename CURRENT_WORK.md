...
## Status: Phase 3 fully complete and merged — no active branch

`phase-3/recent-performance-factor` (weight-tuning tool + the recent-performance factor it led to) is merged to `main` (PR #13) and the local/remote branch is cleaned up. Everything described below is on `main`.
...
**Start/sit recommendation engine**, **player trending endpoint**, **backtest validation**, **weight tuning + recent-performance factor** — see `CLAUDE.md` for the full durable writeup. The *original* backtest run showed essentially zero correlation between predicted score and real fantasy output — an honest, expected result for a `v1` model whose weights were hand-picked and never checked against outcomes.

### Weight tuning + recent-performance factor (PR #13)

- `analytics.backtest.RecommendationMatcher` (shared matching logic, extracted from `BacktestService`), `SimpleLinearRegression` (hand-rolled univariate OLS), `WeightTuningService`, `GET /api/recommendations/tune-weights?season=`.
...
Re-ran the full 2025 season backtest after adding the new factor:
...
Every Phase 3 checklist item from `docs/development-plan.md` is closed. Two real directions from here, neither started yet:

1. **Keep improving model quality** — investigate the `MatchupFactorCalculator` averaging-window hypothesis above, or address the weak `USAGE` factor.
2. **Move to the next phase** — waiver-wire and trade analysis (`docs/development-plan.md`), which reuse this same factor engine. Likely the bigger win now that start/sit has a validated, working model.

Remote branches `phase-2/defense-vs-position-stats`, `phase-2/weather-forecasts`, `phase-2/betting-lines`, `phase-3/start-sit-scoring-engine`, `phase-3/player-trending-endpoint`, and `phase-3/backtest-validation` still exist on origin from prior slices (not yet deleted — user has deferred that cleanup each time). `phase-3/recent-performance-factor` has been deleted both locally and (via the GitHub merge) remotely.
